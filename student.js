...
function renderTopics() {
  ...
  list.innerHTML = allTopics.map((t, idx) => {
    ...
    const locked = idx > 0 && !completedTopicIds.has(allTopics[idx - 1].id);
    return `
      <div class="topic-card glass p-4 rounded-lg ${done ? 'opacity-70' : ''} ${locked ? 'opacity-40' : ''}">
        <div class="flex justify-between items-center">
          <div>
            <span class="text-xs" style="color:rgba(255,255,255,0.6)">${done ? '✅ Selesai' : locked ? '🔒 Terkunci' : '📖 Tersedia'}</span>
            <h4 class="text-md font-semibold text-white mt-1">${esc(t.title)}</h4>
            ${t.content_url ? `<a href="${esc(t.content_url)}" target="_blank" rel="noopener" class="text-xs underline" style="color:rgba(255,255,255,0.7)">Buka materi →</a>` : ''}
          </div>
          <button
            class="btn btn-primary mt-2 text-sm"
            onclick="openTopic('${t.id}')"
            ${locked ? 'disabled title="Selesaikan topik sebelumnya dahulu"' : ''}
...
async function loadProgress() {
  ...
  if (!error && data) {
    completedTopicIds = new Set(data.map(r => r.topic_id));
  }
  updateProgressUI();
}
...
function updateProgressUI() {
  ...
  if (examSection) {
    // Exam unlocked only when ALL topics completed (min 12 enforced by admin)
