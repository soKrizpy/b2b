...
// Single HTML escaping function — used everywhere
...
function renderPagination(page, totalPages, onPageChange) {
  ...
  html += `<span class="page-info">${page * 10 + 1}–${Math.min((page + 1) * 10, (page + 1) * 10)} dari ${totalPages * 10 > total ? total : (page + 1) * 10 > total ? total : (page + 1) * 10}</span>`;
