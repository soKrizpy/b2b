...
async function checkAuth() {
  ...
  const profile = await apiHandler.handle(
    sbClient.from("profiles").select("role").eq("id", user.id).single(),
  );
...
async function loadStudents() {
  const data = await apiHandler.handle(
    sbClient.from("profiles").select("*").eq("role", "student").order("full_name"),
  );
  if (!data) return;
...
function renderStudentList() {
  ...
  list.innerHTML = html;
  mini.innerHTML = allStudents.length
    ? allStudents
        .slice(0, 4)
        .map(
          (s) => `
          <div class="item-row clickable" onclick="selectStudent('${s.id}'); switchAdminTab('students')">
            <h3 class="font-bold text-primary">${esc(s.full_name)}</h3>
            <p class="text-secondary text-sm mt-1">Klik untuk detail</p>
          </div>`,
        )
        .join("")
    : `<div class="empty-state">${icon("users", "icon-lg")}<h3>Belum ada siswa</h3></div>`;
...
function renderStudentDetail() {
  ...
  const next = studentSchedules
    .filter((s) => s.status === "upcoming" && new Date(s.start_time) >= new Date())
    .sort((a, b) => new Date(a.start_time) - new Date(b.start_time))[0];
  const completed = studentSchedules.filter((s) => s.status === "completed").length;
  const missed = studentSchedules.filter((s) => s.attendance_status === "missed").length;
...
function renderOverview() {
  ...
  list.innerHTML = todaySchedules.length
    ? todaySchedules.map((s) => renderScheduleCard(s, { compact: true })).join("")
...
async function saveSchedule() {
  ...
  if (id) {
    await apiHandler.handle(sbClient.from("schedules").update(scheduleData).eq("id", id), async () => {
...
async function editSchedule(id) {
  ...
  document.getElementById("scheduleTime").value = formatDate.toDateTimeLocal(data.start_time);
  document.getElementById("scheduleLink").value = data.meeting_link;
  document.getElementById("scheduleAttendance").value = data.attendance_status || "pending";
...
async function deleteSchedule(id) {
  if (await showConfirm("Hapus Jadwal", "Yakin hapus jadwal ini?", "danger")) {
    await apiHandler.handle(sbClient.from("schedules").delete().eq("id", id), async () => {
...
async function markSchedule(id, status) {
  ...
  await apiHandler.handle(
    sbClient.from("schedules").update({ status, attendance_status: attendance }).eq("id", id),
    async () => {
...
async function loadLearningPath() {
  const studentId = document.getElementById("studentSelect").value || selectedStudentId;
...
async function toggleModule(id, completed) {
  await apiHandler.handle(
    sbClient.from("learning_paths").update({ is_completed: completed }).eq("id", id),
...
async function deleteModule(id) {
  if (await showConfirm("Hapus Materi", "Yakin hapus materi ini?", "danger")) {
    await apiHandler.handle(sbClient.from("learning_paths").delete().eq("id", id), async () => {
...
async function loadRequests() {
  const data = await apiHandler.handle(
    sbClient
      ...
      .select("*, profiles:student_id(full_name), schedules:schedule_id(title,start_time)")
...
async function markAllRead(type) {
  await apiHandler.handle(
    sbClient.from("notifications").update({ is_read: true }).eq("user_id", currentUser.id),
    () => loadNotifications(type),
  );
}
...
async function sendNotificationToStudent(studentId, title, message) {
  await apiHandler.handle(
    sbClient.from("notifications").insert([{ user_id: studentId, title, message, type: "schedule" }]),
  );
}
