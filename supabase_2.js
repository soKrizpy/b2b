...
// INP helper: yield to main thread
function yieldToMain(){return new Promise(r=>{if('scheduler' in window && window.scheduler.yield) window.scheduler.yield().then(r); else setTimeout(r,0);});}
...
function switchStudentTab(tabName) {
  if (tabName === "materials" && !hasCompletedSession) {
    toast("Selesaikan minimal 1 pertemuan dengan guru untuk membuka Materi 📚","error");
    tabName = "home";
  }
  // INP fix: do minimal work in click handler, defer DOM writes
  const btns = document.querySelectorAll(".tab-btn");
  const panels = document.querySelectorAll(".tab-panel");
  // Fast path: toggle without layout thrash
...
async function checkAuth() {
  ...
  refreshInterval = setInterval(async () => {
    if (document.hidden) return; // don't refresh when tab not visible (INP save)
    await Promise.all([
      loadUpcomingSchedules(),
      loadSidebarData(),
      loadHistory(),
